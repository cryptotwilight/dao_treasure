...
interface ITresoreria { 
    ...
    function getApprovedAddresses() view external returns(address [] memory _approvedAddress, string [] memory _addressName);

    function getAvailableBalances() view external returns (string [] memory _currency, uint256 [] memory _balance);

    function getLatestDeals() view external returns (string [] memory _protocol, string [] memory _market, uint256 [] memory _aprapy, uint256 [] memory _totalValueLocked);

    function getHoldings() view external returns (string [] memory protocol, string [] memory _market, 
                                                    uint256 [] memory _rateYield, bool [] memory _isAPY, 
                                                        uint256 [] memory _volume, uint256 [] memory earningsToDate, 
                                                            uint256 [] memory _initialInvestment, uint256 [] memory _investmentDate);

    function getInvestmentHistory() view external returns (string [] memory _protocol, string [] memory _market, 
    ...
    function getWithdrawalHistory() view external returns (string [] memory _currency, uint256 [] memory _volume, 
                                                                uint256[] memory _denominated, uint256[] memory _withdrawalDate, 
                                                                    address [] memory _withdrawer, address [] memory _payee);

    function getSwapHistory() view external returns (string [] memory _fromCurrency, string [] memory _toCurrency, 
